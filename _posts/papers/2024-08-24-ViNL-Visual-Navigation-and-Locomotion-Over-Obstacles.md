...
## Comments
이 연구에서는 **ViNL**은 로봇이 복잡한 실내 환경에서 장애물을 피하면서 목표 지점까지 도달하는 접근 방식을 제시하고 있다. 현재 내가 관심을 갖고 있는 *Vision-and-Language Navigation (VLN)* 연구에서도 로봇이 어떻게 효율적으로 장애물을 회피하고 목표 지점까지 안전하게 도달할 수 있을지를 우선적으로 고려해야할 문제라고 생각한다. 특히, 이 프레임워크에서 활용된 시계열 모델인 *Long Short-Term Memory (LSTM)* 은 *VLN* 연구에서도 텍스트 지시와 관련된 시각적 단서를 기억하고 활용하는 데 매우 유용할 것이라고 생각된다. 이를 잘 활용한다면 더욱 정밀한 동작 계획과 수행이 가능한 시스템을 구축할 수 있을 것 같다는 생각이 들었다. *(찾아보니, 실제로 LSTM을 최근 VLN 연구에서도 많이 활용하고 있었다.)* 이외에도 계층적 학습, 시뮬레이션 활용, 시각적 정보의 활용 등은 향후 나의 연구에 영감을 주는 의미있는 요소들로, 이 논문을 리뷰하며 많은 아이디어를 얻을 수 있었다. 앞으로의 연구에서 이러한 요소들을 적용해 볼 수 있기를 기대하며 글을 마친다.